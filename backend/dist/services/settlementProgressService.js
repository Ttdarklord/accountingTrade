...
exports.SettlementProgressService = void 0;
const connection_1 = require("../database/connection");
class SettlementProgressService {
    /**
     ...
     */
    static calculateProgressForTrades(trades) {
        ...
        trades.forEach(trade => {
            if (!trade.counterparty_id)
                return; // Skip trades without counterparty
     ...
     */
    static calculateProgressForTradeAndCurrency(trade, currency) {
        ...
        if (obligation.amount === 0)
            return 0; // No obligation in this currency
     ...
     */
    static getTradeObligationForCurrency(trade, currency) {
     ...
     */
    static getPaymentEventsForCounterparty(counterpartyId, currency) {
        let query = '';
        let params = [];
        if (currency === 'TOMAN') {
            query = `
      ...
      `;
        ...
        const rawEvents = connection_1.db.prepare(query).all(...params);
        const events = [];
     ...
     */
    static calculateTradeSettlement(trade, currency, obligation, payments) {
        // Get all trades for this counterparty in chronological order (for FIFO)
        const allTrades = connection_1.db.prepare(`
      SELECT * FROM trades 
      WHERE counterparty_id = ? 
      ORDER BY created_at ASC
    `).all(trade.counterparty_id);
...
exports.SettlementProgressService = SettlementProgressService;
//# sourceMappingURL=settlementProgressService.js.map