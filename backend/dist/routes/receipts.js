...
const createReceiptSchema = zod_1.z.object({
    // Common fields
    tracking_last_5: zod_1.z.string().length(5),
    amount: zod_1.z.number().positive(),
    currency: zod_1.z.enum(['AED', 'TOMAN']),
    receipt_date: zod_1.z.string(),
    notes: zod_1.z.string().optional(),
    // TOMAN receipt fields (conditional)
    payer_id: zod_1.z.number().positive().optional(),
    receiver_account_id: zod_1.z.number().positive().optional(),
    // AED receipt fields (conditional)
    receipt_type: zod_1.z.enum(['pay', 'receive']).optional(),
    trading_party_id: zod_1.z.number().positive().optional(),
    individual_name: zod_1.z.string().min(1).optional()
}).refine(data => {
...
const deleteReceiptSchema = zod_1.z.object({
    reason: zod_1.z.string().min(1, "Deletion reason is required"),
    reason_category: zod_1.z.enum(['duplicate', 'funds_returned', 'receipt_not_landed', 'data_error', 'other']),
    deleted_by: zod_1.z.string().optional()
});
// Validation schema for receipt restoration
const restoreReceiptSchema = zod_1.z.object({
    reason: zod_1.z.string().min(1, "Restoration reason is required"),
    restored_by: zod_1.z.string().optional()
});
...
router.get('/', async (req, res) => {
    ...
    `;
        ...
        const receipts = connection_1.default.prepare(query).all(...params);
        res.json({
            success: true,
            data: receipts
        });
...
router.get('/:id', async (req, res) => {
    try {
        ...
        const receipt = connection_1.default.prepare(`
      SELECT 
        pr.*,
        tp_payer.name as payer_name,
        tp_trading.name as trading_party_name,
...
router.post('/', async (req, res) => {
    try {
        ...
        let tradingParty = null;
        if (validatedData.currency === 'TOMAN') {
            // Check if payer exists
            payer = connection_1.default.prepare('SELECT * FROM trading_parties WHERE id = ?').get(validatedData.payer_id);
            if (!payer) {
      ...
      `).get(validatedData.receiver_account_id);
            ...
            tradingParty = connection_1.default.prepare('SELECT * FROM trading_parties WHERE id = ?').get(validatedData.trading_party_id);
            if (!tradingParty) {
                return res.status(400).json({
                    success: false,
      ...
      `);
            const result = stmt.run(validatedData.payer_id || null, validatedData.receiver_account_id || null, validatedData.tracking_last_5, validatedData.amount, validatedData.receipt_date, validatedData.notes || null, validatedData.currency, validatedData.receipt_type || null, validatedData.trading_party_id || null, validatedData.individual_name || null);
            ...
            if (validatedData.currency === 'TOMAN') {
                ...
                (0, counterparts_1.updateCounterpartBalance)(validatedData.payer_id, 'TOMAN', validatedData.amount, // positive = credit to payer (they owe us less)
                'RECEIPT', description + ' - Payment made', validatedData.receipt_date, undefined, receiptId, false // Don't use own transaction - we're managing it here
                );
                // Decrease receiver's balance (we owe them less)
                (0, counterparts_1.updateCounterpartBalance)(receiverAccount.counterpart_id, 'TOMAN', -validatedData.amount, // negative = debit to receiver (we owe them less)
                'RECEIPT', description + ' - Payment received', validatedData.receipt_date, undefined, receiptId, false // Don't use own transaction - we're managing it here
                );
            }
            else {
                ...
                (0, counterparts_1.updateCounterpartBalance)(validatedData.trading_party_id, 'AED', amount, 'RECEIPT', description + ` - AED ${validatedData.receipt_type} ${validatedData.individual_name}`, validatedData.receipt_date, undefined, receiptId, false // Don't use own transaction - we're managing it here
                );
            }
            ...
            const journalStmt = connection_1.default.prepare(`
      ...
      `);
            ...
            const journalResult = journalStmt.run(entryNumber, receiptId, description_text, validatedData.receipt_date);
            const journalEntryId = journalResult.lastInsertRowid;
            // Create journal entry lines
            const lineStmt = connection_1.default.prepare(`
      ...
      `);
            if (validatedData.currency === 'TOMAN') {
                ...
                lineStmt.run(journalEntryId, '1101', 'Counterpart Receivables - Toman', validatedData.amount, 0, 'TOMAN', validatedData.payer_id);
                // Credit: Counterpart Payables (money we owe to receiver increased)
                lineStmt.run(journalEntryId, '2001', 'Counterpart Payables - Toman', 0, validatedData.amount, 'TOMAN', receiverAccount.counterpart_id);
            }
            else {
                // For AED receipts (cash transactions)
                if (validatedData.receipt_type === 'receive') {
                    // Debit: Cash AED (increase cash)
                    lineStmt.run(journalEntryId, '1002', 'Cash - AED', validatedData.amount, 0, 'AED', null);
                    // Credit: Counterpart Receivables (decrease what they owe us)
                    lineStmt.run(journalEntryId, '1102', 'Counterpart Receivables - AED', 0, validatedData.amount, 'AED', validatedData.trading_party_id);
                }
                else {
                    // Credit: Cash AED (decrease cash)
                    lineStmt.run(journalEntryId, '1002', 'Cash - AED', 0, validatedData.amount, 'AED', null);
                    // Debit: Counterpart Payables (decrease what we owe them)
                    lineStmt.run(journalEntryId, '2002', 'Counterpart Payables - AED', validatedData.amount, 0, 'AED', validatedData.trading_party_id);
                }
            }
            connection_1.default.exec('COMMIT');
            // Process FIFO settlement after successful receipt creation
            try {
                settlementService_1.SettlementService.processReceipt(receiptId);
            }
            ...
            const newReceipt = connection_1.default.prepare(`
        SELECT 
          pr.*,
          tp_payer.name as payer_name,
          tp_trading.name as trading_party_name,
    ...
    `).get(receiptId);
        ...
        try {
            ...
            if (receipt.currency === 'TOMAN') {
                ...
                (0, counterparts_1.updateCounterpartBalance)(receipt.payer_id, 'TOMAN', -receipt.amount, // negative = debit (they owe us more)
                'RECEIPT', description + ' - Payment reversed', new Date().toISOString().split('T')[0], undefined, receiptId, false // Don't use own transaction - we're managing it here
                );
                // Only update receiver balance if there's a valid receiver counterpart
                if (receipt.receiver_counterpart_id) {
                    (0, counterparts_1.updateCounterpartBalance)(receipt.receiver_counterpart_id, 'TOMAN', receipt.amount, // positive = credit (we owe them more)
                    'RECEIPT', description + ' - Receipt reversed', new Date().toISOString().split('T')[0], undefined, receiptId, false // Don't use own transaction - we're managing it here
                    );
                }
            }
            else {
                ...
                (0, counterparts_1.updateCounterpartBalance)(receipt.trading_party_id, 'AED', amount, // Reverse the original transaction
                'RECEIPT', description + ' - AED receipt reversed', new Date().toISOString().split('T')[0], undefined, receiptId, false // Don't use own transaction - we're managing it here
                );
            }
        ...
        VALUES (?, ?, ?, ?)
      `);
            const journalResult = journalStmt.run(entryNumber, receiptId, description, new Date().toISOString().split('T')[0]);
            const journalEntryId = journalResult.lastInsertRowid;
      ...
      `);
            if (receipt.currency === 'TOMAN') {
                ...
                lineStmt.run(journalEntryId, '1101', 'Counterpart Receivables - Toman', 0, receipt.amount, 'TOMAN', receipt.payer_id);
                // Debit: Counterpart Payables (money we owe to receiver decreased)
                if (receipt.receiver_counterpart_id) {
                    lineStmt.run(journalEntryId, '2001', 'Counterpart Payables - Toman', receipt.amount, 0, 'TOMAN', receipt.receiver_counterpart_id);
                }
            }
            else {
                // For AED receipts (cash transactions)
                if (receipt.receipt_type === 'receive') {
                    // Credit: Cash AED (decrease cash)
                    lineStmt.run(journalEntryId, '1002', 'Cash - AED', 0, receipt.amount, 'AED', null);
                    // Debit: Counterpart Receivables (increase what they owe us)
                    lineStmt.run(journalEntryId, '1102', 'Counterpart Receivables - AED', receipt.amount, 0, 'AED', receipt.trading_party_id);
                }
                else {
                    // Debit: Cash AED (increase cash)
                    lineStmt.run(journalEntryId, '1002', 'Cash - AED', receipt.amount, 0, 'AED', null);
                    // Credit: Counterpart Payables (increase what we owe them)
                    lineStmt.run(journalEntryId, '2002', 'Counterpart Payables - AED', 0, receipt.amount, 'AED', receipt.trading_party_id);
                }
            }
            // Mark receipt as deleted (soft delete)
            const deleteStmt = connection_1.default.prepare(`
        ...
        WHERE id = ?
      `);
            deleteStmt.run(deletionTime, validatedData.reason, validatedData.reason_category, validatedData.deleted_by || 'Unknown', receiptId);
            connection_1.default.exec('COMMIT');
    ...
    `).get(receiptId);
        ...
        try {
            ...
            if (receipt.currency === 'TOMAN') {
                ...
                (0, counterparts_1.updateCounterpartBalance)(receipt.payer_id, 'TOMAN', receipt.amount, // positive = credit to payer (they owe us less)
                'RECEIPT', description + ' - Payment restored', new Date().toISOString().split('T')[0], undefined, receiptId, false);
                // Decrease receiver's balance (we owe them less again)
                if (receipt.receiver_counterpart_id) {
                    (0, counterparts_1.updateCounterpartBalance)(receipt.receiver_counterpart_id, 'TOMAN', -receipt.amount, // negative = debit to receiver (we owe them less)
                    'RECEIPT', description + ' - Receipt restored', new Date().toISOString().split('T')[0], undefined, receiptId, false);
                }
            }
            else {
                // For AED receipts, re-apply the original transaction
                const amount = receipt.receipt_type === 'receive' ? receipt.amount : -receipt.amount;
                (0, counterparts_1.updateCounterpartBalance)(receipt.trading_party_id, 'AED', amount, // Re-apply the original transaction
                'RECEIPT', description + ' - AED receipt restored', new Date().toISOString().split('T')[0], undefined, receiptId, false);
            }
      ...
      `);
            if (receipt.currency === 'TOMAN') {
                // Debit: Counterpart Receivables (money owed to us by payer decreased)
                lineStmt.run(journalEntryId, '1101', 'Counterpart Receivables - Toman', receipt.amount, 0, 'TOMAN', receipt.payer_id);
                // Credit: Counterpart Payables (money we owe to receiver increased)
                if (receipt.receiver_counterpart_id) {
                    lineStmt.run(journalEntryId, '2001', 'Counterpart Payables - Toman', 0, receipt.amount, 'TOMAN', receipt.receiver_counterpart_id);
                }
            }
            else {
                // For AED receipts (cash transactions)
                if (receipt.receipt_type === 'receive') {
                    ...
                    lineStmt.run(journalEntryId, '1002', 'Cash - AED', receipt.amount, 0, 'AED', null);
                    // Credit: Counterpart Receivables (decrease what they owe us)
                    lineStmt.run(journalEntryId, '1102', 'Counterpart Receivables - AED', 0, receipt.amount, 'AED', receipt.trading_party_id);
                }
                else {
                    // Credit: Cash AED (decrease cash)
                    lineStmt.run(journalEntryId, '1002', 'Cash - AED', 0, receipt.amount, 'AED', null);
                    // Debit: Counterpart Payables (decrease what we owe them)
                    lineStmt.run(journalEntryId, '2002', 'Counterpart Payables - AED', receipt.amount, 0, 'AED', receipt.trading_party_id);
                }
            }
            // Mark receipt as restored
            const restoreStmt = connection_1.default.prepare(`
        UPDATE payment_receipts 
        SET is_deleted = 0, 
            is_restored = 1,
            restored_at = ?, 
        ...
        WHERE id = ?
      `);
            restoreStmt.run(restorationTime, validatedData.reason, validatedData.restored_by || 'Unknown', receiptId);
            connection_1.default.exec('COMMIT');
