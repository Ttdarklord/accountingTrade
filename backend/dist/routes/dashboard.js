...
router.get('/', async (req, res) => {
    try {
        ...
        const balanceQuery = connection_1.default.prepare(`
      SELECT 
        jel.currency,
    ...
    `).all();
        ...
        // Get pending trades
        const pendingTrades = connection_1.default.prepare(`
    ...
    `).all();
        ...
        const outstandingPositions = connection_1.default.prepare(`
    ...
    `).all();
        ...
        const recentTransactions = connection_1.default.prepare(`
      SELECT t.*, tp.name as counterparty_name
      FROM trades t
      LEFT JOIN trading_parties tp ON t.counterparty_id = tp.id
      ORDER BY t.created_at DESC
    ...
    `).all();
        ...
        const profitSummary = connection_1.default.prepare(`
      SELECT 
    ...
    `).get();
        ...
        const tradeStatusCounts = connection_1.default.prepare(`
      SELECT 
    ...
    `).get();
        const dashboardData = {
            ...
            profit_summary: {
                total_profit_toman: profitSummary?.total_profit_toman || 0,
                total_profit_aed: profitSummary?.total_profit_aed || 0,
                monthly_profit_toman: profitSummary?.monthly_profit_toman || 0,
                monthly_profit_aed: profitSummary?.monthly_profit_aed || 0
            },
            trade_status_counts: {
                pending_count: tradeStatusCounts?.pending_count || 0,
                partial_count: tradeStatusCounts?.partial_count || 0,
                completed_count: tradeStatusCounts?.completed_count || 0
            }
        };
        res.json({
            success: true,
