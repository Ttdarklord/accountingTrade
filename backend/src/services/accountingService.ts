...
export class AccountingService {
  ...
  static createPurchaseEntry(
    ...
    const entryNumber = `JE-${format(new Date(), 'yyyyMMdd')}-${uuidv4().slice(0, 8)}`;
    ...
    const lines: Omit<JournalEntryLine, 'id' | 'created_at'>[] = [];
  ...
  static getJournalEntry(journalEntryId: number): JournalEntry {
    ...
    const entry = getEntry.get(journalEntryId) as JournalEntry;
    const lines = getLines.all(journalEntryId) as JournalEntryLine[];
   ...
   */
  static updateCompanyBalance(currency: Currency, amount: number, isSafeBalance = false): void {
